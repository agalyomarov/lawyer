...
wpcr3.submit = function(e) {
	...
	if (wpcr3.mousemove_total <= wpcr3.mousemove_need || wpcr3.keypress_total <= wpcr3.keypress_need) {
		alert('Вы не прошли нашу проверку на обнаружение людей. Код '+wpcr3.mousemove_total+','+wpcr3.keypress_total);
		return false;
	}
	...
	if (c1_fail || c3_fail || fake_fail) {
		alert('Вы не прошли нашу проверку на обнаружение людей. Код '+c1_fail+','+c3_fail+','+fake_fail);
		return false;
	}
	if (c2_fail) {
		alert('Вы должны установить флажок, чтобы подтвердить, что вы человек.');
		return false;
	}
	...
	$.each(fields, function(i,v) {
		...
		if (v.hasClass('wpcr3_required') && $.trim(v.val()).length === 0) {
			...
			if (label.length) {
				...
				err = 'Не заполнено обязательное поле.';
			}
			req.push(err);
		}
	});
	...
	wpcr3.ajaxPost(parent, ajaxData, function(err, rtn) {
		...
		alert('Спасибо! Ваш отзыв был получен и скоро будет опубликован.');
		$(window).scrollTop(0);
		wpcr3.clearFields();
